...
from atni.parsers.zteumts_asn1.zteumts_asn1_schema import *
from atni.parsers.zteumts_asn1.zteumts_parsed import *
from atni.parsers.zteumts_asn1.zteumts_asn_gen import CallEventRecord
from pyspark import StorageLevel
from atni.parsers.utilities import *
from atni.parsers.zteumts_asn1.zteumts_stats import ZTEUMTSStats
from atni.parsers.zteumts_asn1.process_records.zteumts_process_commonequiprecord import ProcessZTEumtsCommonEquipRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_incgatewayrecord import ProcessZTEumtsIncGatewayRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_mcfcallrecord import ProcessZTEumtsMCFCallRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_mocallrecord import ProcessZTEumtsMOCallRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_mosmsrecord import ProcessZTEumtsMOSMSRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_mtsmsrecord import ProcessZTEumtsMTSMSRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_mtcallrecord import ProcessZTEumtsMTCallRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_mtlcsrecord import ProcessZTEumtsmtLCSRecordRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_outgatewayrecord import ProcessZTEumtsOutGatewayRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_ssactionrecord import ProcessZTEumtsSSActionRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_ussdrecord import ProcessZTEumtsUSSDRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_gsicphrecord import ProcessZTEumtsGSICPHRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_hlrintrecord import ProcessZTEumtsHLRIntRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_molcsrecord import ProcessZTEumtsmoLCSRecordRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_mtrfrecord import ProcessZTEumtsMTRFRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_ncscphrecord import ProcessZTEumtsNCSCPHRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_nilcsrecord import ProcessZTEumtsNILCSRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_vigrecord import ProcessZTEumtsVIGRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_mcfcphrecord import ProcessZTEumtsMCFCPHRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_tcicphrecord import ProcessZTEumtsTCICPHRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_mscsrvccrecord import ProcessZTEumtsMSCSRVCCRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_roamrecord import ProcessZTEumtsROAMRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_mocphrecord import ProcessZTEumtsMOCPHRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_moecallrecord import ProcessZTEumtsMOECallRecord
from atni.parsers.zteumts_asn1.process_records.zteumts_process_termcamelintrecord import \
...
def save_zteumts_data(sc, sqlContext, cdr_commonequip_rdd, cdr_incgateway_rdd, cdr_mcfcall_rdd,
    ...
    if cdr_gsicph_rdd.count() > 0:
        insert_records(sqlContext, cdr_gsicph_rdd, get_gsicphrecord_schema(), ZTEUMTS_GSICPH_TABLE_NAME)
        print "############### Saved ZTEUMTS_GSICPH_TABLE_NAME ######################"
    if cdr_hlrint_rdd.count() > 0:
        insert_records(sqlContext, cdr_hlrint_rdd, get_hlrintrecord_schema(), ZTEUMTS_HLRINT_TABLE_NAME)
        print "############### Saved ZTEUMTS_HLRINT_TABLE_NAME ######################"
    if cdr_molcs_rdd.count() > 0:
        insert_records(sqlContext, cdr_molcs_rdd, get_molcs_schema(), ZTEUMTS_MOLCS_TABLE_NAME)
        print "############### Saved ZTEUMTS_MOLCS_TABLE_NAME ######################"
    if cdr_mtrf_rdd.count() > 0:
        insert_records(sqlContext, cdr_mtrf_rdd, get_mtrf_schema(), ZTEUMTS_MTRF_TABLE_NAME)
        print "############### Saved ZTEUMTS_MTRF_TABLE_NAME ######################"
    if cdr_ncscph_rdd.count() > 0:
        insert_records(sqlContext, cdr_ncscph_rdd, get_ncscph_schema(), ZTEUMTS_NCSCPH_TABLE_NAME)
        print "############### Saved ZTEUMTS_NCSCPH_TABLE_NAME ######################"
    if cdr_nilcs_rdd.count() > 0:
        insert_records(sqlContext, cdr_nilcs_rdd, get_nilcs_schema(), ZTEUMTS_NILCS_TABLE_NAME)
        print "############### Saved ZTEUMTS_NILCS_TABLE_NAME ######################"
    if cdr_vig_rdd.count() > 0:
        insert_records(sqlContext, cdr_vig_rdd, get_vig_schema(), ZTEUMTS_VIG_TABLE_NAME)
        print "############### Saved ZTEUMTS_VIG_TABLE_NAME ######################"
    if cdr_mcfcph_rdd.count() > 0:
        insert_records(sqlContext, cdr_mcfcph_rdd, get_mcfcph_schema(), ZTEUMTS_MCFCPH_TABLE_NAME)
        print "############### Saved ZTEUMTS_MCFCPH_TABLE_NAME ######################"
    if cdr_tcicph_rdd.count() > 0:
        insert_records(sqlContext, cdr_tcicph_rdd, get_tcicph_schema(), ZTEUMTS_TCICPH_TABLE_NAME)
        print "############### Saved ZTEUMTS_TCICPH_TABLE_NAME ######################"
    if cdr_mscsrvcc_rdd.count() > 0:
        insert_records(sqlContext, cdr_mscsrvcc_rdd, get_mscsrvcc_schema(), ZTEUMTS_MSCSRVCC_TABLE_NAME)
        print "############### Saved ZTEUMTS_MSCSRVCC_TABLE_NAME ######################"
    if cdr_roam_rdd.count() > 0:
        insert_records(sqlContext, cdr_roam_rdd, get_roam_schema(), ZTEUMTS_ROAM_TABLE_NAME)
        print "############### Saved ZTEUMTS_ROAM_TABLE_NAME ######################"
    if cdr_mocph_rdd.count() > 0:
        insert_records(sqlContext, cdr_mocph_rdd, get_mocph_schema(), ZTEUMTS_MOCPH_TABLE_NAME)
        print "############### Saved ZTEUMTS_MOCPH_TABLE_NAME ######################"
    if cdr_moecall_rdd.count() > 0:
        insert_records(sqlContext, cdr_moecall_rdd, get_moecall_schema(), ZTEUMTS_MOECALL_TABLE_NAME)
        print "############### Saved ZTEUMTS_MOECALL_TABLE_NAME ######################"
    if cdr_termcamelint_rdd.count() > 0:
        insert_records(sqlContext, cdr_termcamelint_rdd, get_termcamelint_schema(), ZTEUMTS_TERMCAMELINT_TABLE_NAME)
...
def main():
    ...
    cdr_whole = sc.binaryFiles('hdfs://quickstart.cloudera:8020/user/cloudera/B2016061536247.dat')
...
def parse_zteumts_file(hdfs_file_name, content, iteration_id):
    ...
    # check_sum = get_file_check_sum(str(_absoluteFileName))
    check_sum = 1234789
